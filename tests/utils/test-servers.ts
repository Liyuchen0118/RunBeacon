...
export abstract class BaseTestServer extends EventEmitter {
  ...
  protected addConnection(id: string, socket: any): void {
    ...
    if (this.config.logging) {
      console.log(
        `[TestServer] New connection: ${id} from ${info.remoteAddress}:${info.remotePort}`
      );
    }
  }
  ...
  protected updateConnectionActivity(
    id: string,
  ...
  protected shouldSimulateError(): boolean {
    return this.config.behavior?.errorRate
      ? Math.random() < this.config.behavior.errorRate
      : false;
  }

  protected shouldDropConnection(): boolean {
    return this.config.behavior?.dropRate
      ? Math.random() < this.config.behavior.dropRate
      : false;
  }
    ...
    return new Promise((resolve, reject) => {
      ...
      this.server.listen(
        ...
        () => {
          ...
          if (this.config.logging) {
            console.log(
              `[TCPTestServer] Listening on ${address.address}:${address.port}`
            );
          }
  ...
  private setupDefaultRoutes(): void {
    ...
    this.addRoute('GET', '/delay/:ms', (req, res) => {
      ...
      setTimeout(() => {
        res.writeHead(200, { 'Content-Type': 'application/json' });
        res.end(
          JSON.stringify({ message: `Delayed response after ${delay}ms` })
        );
      }, delay);
    });
    ...
    return new Promise((resolve, reject) => {
      const requestHandler = async (
        ...
        const handler =
          ...
          this.routes.get(`${req.method} ${req.url?.split('/')[1]}`);
      ...
      this.server.listen(
        ...
        () => {
          ...
          if (this.config.logging) {
            console.log(
              `[HTTPTestServer] Listening on ${address.address}:${address.port}`
            );
          }
    ...
    return new Promise((resolve, reject) => {
      ...
      this.server.listen(
        ...
        () => {
          ...
          if (this.config.logging) {
            console.log(
              `[WebSocketTestServer] Listening on ${address.address}:${address.port}`
            );
          }
    ...
    return new Promise((resolve, reject) => {
      ...
      this.server.listen(
        ...
        () => {
          ...
          if (this.config.logging) {
            console.log(
              `[SSHTestServer] Listening on ${address.address}:${address.port}`
            );
          }
...
export class TestServerBuilder {
  ...
  withBehavior(
    behavior: NonNullable<TestServerConfig['behavior']>
  ): TestServerBuilder {
...
export const createTestServer = (): TestServerBuilder =>
