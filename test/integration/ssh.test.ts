...
describeIfHardware('SSH Integration Tests', () => {
  ...
  beforeAll(async () => {
    ...
    // Create intentionally fake key-shaped test data without embedding credentials.
    testKeyPath = join(tmpdir(), 'test_ssh_key');
    const fakeKeyLabel = ['OPENSSH', 'PRIVATE KEY'].join(' ');
    writeFileSync(testKeyPath, `-----BEGIN ${fakeKeyLabel}-----
